...
public class DataConverterBedBlock extends DataFix {
    ...
    private <TE> TypeRewriteRule a(Type<?> type, ListType<TE> listtype) {
        ...
        boolean flag = true;
        return TypeRewriteRule.seq(this.fixTypeEverywhere("InjectBedBlockEntityType", this.getInputSchema().findChoiceType(DataConverterTypes.j), this.getOutputSchema().findChoiceType(DataConverterTypes.j), (var0) -> {
            return (pair) -> {
                return pair;
            };
            ...
            for(int k = 0; k < list.size(); ++k) {
                ...
                Stream stream = ((Stream)dynamic1.get("Blocks").flatMap(Dynamic::getStream).orElse(Stream.empty())).map((dynamic2) -> {
                    return ((Number)dynamic2.getNumberValue().orElse(0)).intValue();
                });
                int i1 = 0;

                for(int j1 : stream::iterator) {
                    if (416 == (j1 & 255) << 4) {
                        int k1 = i1 & 15;
                        int l1 = i1 >> 8 & 15;
                        int i2 = i1 >> 4 & 15;
                        ...
                        arraylist.add(((Optional)type1.read(dynamic1.createMap(hashmap)).getSecond()).orElseThrow(() -> {
