...
public class TileEntityTypes<T extends TileEntity> {
    ...
    public static <T extends TileEntity> TileEntityTypes<T> a(String s, TileEntityTypes.a<T> tileentitytypes$a) {
        Type type = null;

        try {
            type = DataConverterRegistry.a().getSchema(DataFixUtils.makeKey(1628)).getChoiceType(DataConverterTypes.j, s);
        } catch (IllegalStateException illegalstateexception) {
            if (SharedConstants.b) {
                throw illegalstateexception;
            }

            z.warn("No data fixer registered for block entity {}", s);
        }

        TileEntityTypes tileentitytypes = tileentitytypes$a.a(type);
        IRegistry.BLOCK_ENTITY_TYPE.a(new MinecraftKey(s), tileentitytypes);
    ...
    static TileEntity a(String s) {
        TileEntityTypes tileentitytypes = IRegistry.BLOCK_ENTITY_TYPE.get(new MinecraftKey(s));
    ...
    public static final class a<T extends TileEntity> {
        ...
        public static <T extends TileEntity> TileEntityTypes.a<T> a(Supplier<? extends T> supplier) {
            return new TileEntityTypes.a<T>(supplier);
        }

        public TileEntityTypes<T> a(Type<?> type) {
            return new TileEntityTypes<T>(this.a, type);
        }
    }
}
