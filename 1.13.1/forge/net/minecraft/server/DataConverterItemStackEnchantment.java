    ...
    private Dynamic<?> a(Dynamic<?> dynamic) {
        Optional optional = dynamic.get("ench").flatMap(Dynamic::getStream).map((stream) -> {
            return stream.map((dynamic1) -> {
                return dynamic1.set("id", dynamic1.createString((String)a.getOrDefault(dynamic1.getInt("id"), "null")));
            });
        ...
        return dynamic.update("StoredEnchantments", (dynamic1) -> {
            return (Dynamic)DataFixUtils.orElse(dynamic1.getStream().map((stream) -> {
                return stream.map((dynamic2) -> {
                    return dynamic2.set("id", dynamic2.createString((String)a.getOrDefault(dynamic2.getInt("id"), "null")));
                });
