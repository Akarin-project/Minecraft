...
public class CommandClear {
    ...
    public static void a(com.mojang.brigadier.CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        commanddispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandDispatcher.a("clear").requires((commandlistenerwrapper) -> {
            return commandlistenerwrapper.hasPermission(2);
        })).executes((commandcontext) -> {
            return a((CommandListenerWrapper)commandcontext.getSource(), Collections.singleton(((CommandListenerWrapper)commandcontext.getSource()).h()), (var0) -> {
                return true;
            }, -1);
        })).then(((RequiredArgumentBuilder)CommandDispatcher.a("targets", ArgumentEntity.d()).executes((commandcontext) -> {
            return a((CommandListenerWrapper)commandcontext.getSource(), ArgumentEntity.f(commandcontext, "targets"), (var0) -> {
                return true;
            }, -1);
        })).then(((RequiredArgumentBuilder)CommandDispatcher.a("item", ArgumentItemPredicate.a()).executes((commandcontext) -> {
        ...
        })).then(CommandDispatcher.a("maxCount", IntegerArgumentType.integer(0)).executes((commandcontext) -> {
    ...
    private static int a(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, Predicate<ItemStack> predicate, int i) throws CommandSyntaxException {
        ...
        if (j == 0) {
            if (collection.size() == 1) {
                throw a.create(((EntityPlayer)collection.iterator().next()).getDisplayName().e());
            } else {
