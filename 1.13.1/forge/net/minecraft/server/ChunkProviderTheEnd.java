...
public class ChunkProviderTheEnd extends ChunkGeneratorAbstract<GeneratorSettingsEnd> {
    ...
    public void a(int ix, int jx, IChunkAccess ichunkaccess) {
        ...
        boolean flag2 = true;
        boolean flag3 = true;
        double[] adouble = this.a(ix * 2, 0, jx * 2, 3, 33, 3);
        ...
        for(int kx = 0; kx < 2; ++kx) {
            for(int lx = 0; lx < 2; ++lx) {
                for(int i1 = 0; i1 < 32; ++i1) {
                    double d0 = 0.25D;
                    double d1 = adouble[((kx + 0) * 3 + lx + 0) * 33 + i1 + 0];
                    double d2 = adouble[((kx + 0) * 3 + lx + 1) * 33 + i1 + 0];
                    double d3 = adouble[((kx + 1) * 3 + lx + 0) * 33 + i1 + 0];
                    double d4 = adouble[((kx + 1) * 3 + lx + 1) * 33 + i1 + 0];
                    double d5 = (adouble[((kx + 0) * 3 + lx + 0) * 33 + i1 + 1] - d1) * 0.25D;
                    double d6 = (adouble[((kx + 0) * 3 + lx + 1) * 33 + i1 + 1] - d2) * 0.25D;
                    double d7 = (adouble[((kx + 1) * 3 + lx + 0) * 33 + i1 + 1] - d3) * 0.25D;
                    double d8 = (adouble[((kx + 1) * 3 + lx + 1) * 33 + i1 + 1] - d4) * 0.25D;
                    ...
                    for(int j1 = 0; j1 < 4; ++j1) {
                        ...
                        for(int k1 = 0; k1 < 8; ++k1) {
                            ...
                            for(int l1 = 0; l1 < 8; ++l1) {
                                ...
                                int j2 = j1 + i1 * 4;
                                int k2 = l1 + lx * 8;
                                ichunkaccess.a(blockposition$mutableblockposition.c(i2, j2, k2), iblockdata, false);
                                d15 += d16;
                            }
    ...
    private double[] a(int ix, int jx, int kx, int lx, int i1, int j1) {
        ...
        double d1 = 684.412D;
        d0 = d0 * 2.0D;
        double[] adouble1 = this.i.a(ix, jx, kx, lx, i1, j1, d0 / 80.0D, 4.277575000000001D, d0 / 80.0D);
        double[] adouble2 = this.g.a(ix, jx, kx, lx, i1, j1, d0, 684.412D, d0);
        double[] adouble3 = this.h.a(ix, jx, kx, lx, i1, j1, d0, 684.412D, d0);
        int k1 = ix / 2;
        int l1 = kx / 2;
        int i2 = 0;

        for(int j2 = 0; j2 < lx; ++j2) {
            for(int k2 = 0; k2 < j1; ++k2) {
                float fx = this.c.c(k1, l1, j2, k2);

                for(int l2 = 0; l2 < i1; ++l2) {
                    double d2 = adouble2[i2] / 512.0D;
                    double d3 = adouble3[i2] / 512.0D;
                    double d5 = (adouble1[i2] / 10.0D + 1.0D) / 2.0D;
                    double d4;
                    if (d5 < 0.0D) {
                        d4 = d2;
                    } else if (d5 > 1.0D) {
                    ...
                    byte b0 = 2;
                    if (l2 > i1 / 2 - b0) {
                        double d6 = (double)((float)(l2 - (i1 / 2 - b0)) / 64.0F);
                    ...
                    b0 = 8;
                    if (l2 < b0) {
                        double d7 = (double)((float)(b0 - l2) / ((float)b0 - 1.0F));
    ...
    public double[] a(int ix, int jx) {
        double d0 = 0.03125D;
        return this.l.a((double)(ix << 4), (double)(jx << 4), 16, 16, 0.0625D, 0.0625D, 1.0D);
    }
