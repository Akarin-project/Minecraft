...
public class EntitySlice<T> extends AbstractSet<T> {
    ...
    public EntitySlice(Class<T> oclass) {
        ...
        for(Class oclass1 : Lists.newArrayList(a)) {
            this.a(oclass1);
        }
    ...
    private void a(T object, Class<?> oclass) {
        List list = (List)this.b.get(oclass);
        if (list == null) {
            this.b.put(oclass, Lists.newArrayList(new Object[]{object}));
        } else {
            list.add(object);
        }
    ...
    public Iterator<T> iterator() {
        return (Iterator<T>)(this.e.isEmpty() ? Collections.emptyIterator() : Iterators.unmodifiableIterator(this.e.iterator()));
    }
