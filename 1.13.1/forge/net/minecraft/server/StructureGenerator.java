...
public abstract class StructureGenerator<C extends WorldGenFeatureConfiguration> extends WorldGenerator<C> {
    ...
    public boolean generate(GeneratorAccess generatoraccess, ChunkGenerator<? extends GeneratorSettings> chunkgenerator, Random random, BlockPosition blockposition, C var5) {
        if (!this.a(generatoraccess)) {
            ...
            for(int k1 = j - i; k1 <= j + i; ++k1) {
                for(int l1 = k - i; l1 <= k + i; ++l1) {
                    ...
                    if (structurestart != a && structurestart.c().a(l, i1, l + 15, i1 + 15)) {
                        ((LongSet)chunkgenerator.getStructureCache(this).computeIfAbsent(j1, (var0) -> {
    ...
    protected StructureStart a(GeneratorAccess generatoraccess, BlockPosition blockposition) {
        label31:
        for(StructureStart structurestart : this.a(generatoraccess, blockposition.getX() >> 4, blockposition.getZ() >> 4)) {
            if (structurestart.b() && structurestart.c().b(blockposition)) {
                Iterator iterator = structurestart.d().iterator();
    ...
    public BlockPosition getNearestGeneratedFeature(World world, ChunkGenerator<? extends GeneratorSettings> chunkgenerator, BlockPosition blockposition, int i, boolean flag) {
        if (!chunkgenerator.getWorldChunkManager().a(this)) {
            ...
            for(SeededRandom seededrandom = new SeededRandom(); l <= i; ++l) {
        ...
        if (!chunkgenerator.getWorldChunkManager().a(this)) {
            ...
            if (structurestart != null) {
                ...
                if (this.a(chunkgenerator, seededrandom, chunkcoordintpair.x, chunkcoordintpair.z)) {
                    ...
                    structurestart = structurestart1.b() ? structurestart1 : a;
                } else {
                    structurestart = a;
                }
    ...
    protected ChunkCoordIntPair a(ChunkGenerator<?> var1, Random var2, int i, int j, int k, int l) {
        return new ChunkCoordIntPair(i + k, j + l);
    }

    protected abstract boolean a(ChunkGenerator<?> var1, Random var2, int var3, int var4);

    protected abstract boolean a(GeneratorAccess var1);

    protected abstract StructureStart a(GeneratorAccess var1, ChunkGenerator<?> var2, SeededRandom var3, int var4, int var5);
