...
public class WorldData {
    ...
    private long e;
    private WorldType f = WorldType.NORMAL;
    private NBTTagCompound g = new NBTTagCompound();
    @Nullable
    private String h;
    private int i;
    private int j;
    ...
    private final Set<String> S = Sets.newLinkedHashSet();
    private final Map<DimensionManager, NBTTagCompound> T = Maps.newIdentityHashMap();
    private NBTTagCompound U;
    private final GameRules V = new GameRules();
    ...
    public WorldData(NBTTagCompound nbttagcompound, DataFixer datafixer, int ix, @Nullable NBTTagCompound nbttagcompound1) {
        this.p = datafixer;
        if (nbttagcompound.hasKeyOfType("Version", 10)) {
            NBTTagCompound nbttagcompound2 = nbttagcompound.getCompound("Version");
        ...
        if (nbttagcompound.hasKeyOfType("generatorName", 8)) {
            String s1 = nbttagcompound.getString("generatorName");
            this.f = WorldType.getType(s1);
            if (this.f == null) {
                ...
                if (nbttagcompound.hasKeyOfType("generatorVersion", 99)) {
                    jx = nbttagcompound.getInt("generatorVersion");
                }
        ...
        if (nbttagcompound.hasKeyOfType("DimensionData", 10)) {
            NBTTagCompound nbttagcompound3 = nbttagcompound.getCompound("DimensionData");

            for(String sx : nbttagcompound3.getKeys()) {
                this.T.put(DimensionManager.a(Integer.parseInt(sx)), nbttagcompound3.getCompound(sx));
            }
        }

        if (nbttagcompound.hasKeyOfType("DataPacks", 10)) {
            NBTTagCompound nbttagcompound4 = nbttagcompound.getCompound("DataPacks");
            NBTTagList nbttaglist = nbttagcompound4.getList("Disabled", 8);

            for(int lx = 0; lx < nbttaglist.size(); ++lx) {
                this.R.add(nbttaglist.getString(lx));
            }

            NBTTagList nbttaglist1 = nbttagcompound4.getList("Enabled", 8);

            for(int kx = 0; kx < nbttaglist1.size(); ++kx) {
                this.S.add(nbttaglist1.getString(kx));
            }
        }
    ...
    public WorldData(WorldSettings worldsettings, String sx) {
        this.p = null;
        this.q = 1628;
    ...
    private void a(NBTTagCompound nbttagcompound, NBTTagCompound nbttagcompound1) {
        ...
        for(String sx : this.S) {
            nbttaglist.add((NBTBase)(new NBTTagString(sx)));
        }

        nbttagcompound4.set("Enabled", nbttaglist);
        NBTTagList nbttaglist1 = new NBTTagList();

        for(String s1 : this.R) {
    ...
    public void setTime(long ix) {
        this.l = ix;
    }

    public void setDayTime(long ix) {
        this.m = ix;
    }
    ...
    public void setThunderDuration(int ix) {
        this.A = ix;
    }
    ...
    public void setWeatherDuration(int ix) {
        this.y = ix;
    }
    ...
    public void h(int ix) {
        this.P = ix;
    }

    public void i(int ix) {
        this.Q = ix;
    }
    ...
    public void a(CrashReportSystemDetails crashreportsystemdetails) {
        ...
        crashreportsystemdetails.a("Level storage version", () -> {
            ...
            return String.format("0x%05X - %s", this.v, sx);
        });
