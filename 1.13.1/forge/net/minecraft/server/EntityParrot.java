...
public class EntityParrot extends EntityPerchable implements EntityBird {
    ...
    private static final Map<EntityTypes<?>, SoundEffect> bP = (Map)SystemUtils.a(Maps.newHashMap(), (hashmap) -> {
    ...
    protected void initAttributes() {
        ...
        this.getAttributeInstance(GenericAttributes.e).setValue((double)0.4F);
        this.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED).setValue((double)0.2F);
    }
    ...
    private static boolean b(World world, Entity entity) {
        if (!entity.isSilent() && world.random.nextInt(50) == 0) {
            List list = world.a(EntityInsentient.class, entity.getBoundingBox().g(20.0D), bM);
            if (!list.isEmpty()) {
    ...
    public boolean a(EntityHuman entityhuman, EnumHand enumhand) {
        ...
        if (!this.isTamed() && bO.contains(itemstack.getItem())) {
            ...
            if (!this.world.isClientSide && !this.F_() && this.isTamed() && this.f(entityhuman)) {
    ...
    private static SoundEffect a(Random random) {
        if (random.nextInt(1000) == 0) {
            ArrayList arraylist = Lists.newArrayList(bP.keySet());
    ...
    public static SoundEffect a(EntityTypes<?> entitytypes) {
        return (SoundEffect)bP.getOrDefault(entitytypes, SoundEffects.ENTITY_PARROT_AMBIENT);
    }
    ...
    public int getVariant() {
        return MathHelper.clamp(this.datawatcher.get(bL), 0, 4);
    }

    public void setVariant(int i) {
        this.datawatcher.set(bL, i);
    }
