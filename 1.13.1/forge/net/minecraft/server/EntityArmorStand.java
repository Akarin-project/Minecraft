...
public class EntityArmorStand extends EntityLiving {
    ...
    public EntityArmorStand(World world) {
        ...
        this.bE = NonNullList.<ItemStack>a(2, ItemStack.a);
        this.bF = NonNullList.<ItemStack>a(4, ItemStack.a);
        this.headPose = bx;
        this.bodyPose = by;
        this.leftArmPose = bz;
        this.rightArmPose = bA;
        this.leftLegPose = bB;
        this.rightLegPose = bC;
    ...
    public final void setSize(float fx, float f1) {
        double d0 = this.locX;
        double d1 = this.locY;
        double d2 = this.locZ;
        float f2 = this.isMarker() ? 0.0F : (this.isBaby() ? 0.5F : 1.0F);
        super.setSize(fx * f2, f1 * f2);
        this.setPosition(d0, d1, d2);
    }
    ...
    protected void x_() {
        ...
        this.datawatcher.register(b, bx);
        this.datawatcher.register(c, by);
        this.datawatcher.register(d, bz);
        this.datawatcher.register(e, bA);
        this.datawatcher.register(f, bB);
        this.datawatcher.register(g, bC);
    }
    ...
    public ItemStack getEquipment(EnumItemSlot enumitemslot) {
        ...
        case ARMOR:
            return this.bF.get(enumitemslot.b());
        default:
            return ItemStack.a;
        }
    }
    ...
    public void b(NBTTagCompound nbttagcompound) {
        ...
        for(ItemStack itemstack1 : this.bE) {
    ...
    public void a(NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.hasKeyOfType("HandItems", 9)) {
            ...
            for(int j = 0; j < this.bE.size(); ++j) {
                this.bE.set(j, ItemStack.a(nbttaglist1.getCompound(j)));
            }
        }
    ...
    private void g(NBTTagCompound nbttagcompound) {
        ...
        this.setHeadPose(nbttaglist.isEmpty() ? bx : new Vector3f(nbttaglist));
        NBTTagList nbttaglist1 = nbttagcompound.getList("Body", 5);
        this.setBodyPose(nbttaglist1.isEmpty() ? by : new Vector3f(nbttaglist1));
        NBTTagList nbttaglist2 = nbttagcompound.getList("LeftArm", 5);
        this.setLeftArmPose(nbttaglist2.isEmpty() ? bz : new Vector3f(nbttaglist2));
        NBTTagList nbttaglist3 = nbttagcompound.getList("RightArm", 5);
        this.setRightArmPose(nbttaglist3.isEmpty() ? bA : new Vector3f(nbttaglist3));
        NBTTagList nbttaglist4 = nbttagcompound.getList("LeftLeg", 5);
        this.setLeftLegPose(nbttaglist4.isEmpty() ? bB : new Vector3f(nbttaglist4));
        NBTTagList nbttaglist5 = nbttagcompound.getList("RightLeg", 5);
        this.setRightLegPose(nbttaglist5.isEmpty() ? bC : new Vector3f(nbttaglist5));
    }

    private NBTTagCompound z() {
        NBTTagCompound nbttagcompound = new NBTTagCompound();
        if (!bx.equals(this.headPose)) {
            nbttagcompound.set("Head", this.headPose.a());
        }

        if (!by.equals(this.bodyPose)) {
            nbttagcompound.set("Body", this.bodyPose.a());
        }

        if (!bz.equals(this.leftArmPose)) {
            nbttagcompound.set("LeftArm", this.leftArmPose.a());
        }

        if (!bA.equals(this.rightArmPose)) {
            nbttagcompound.set("RightArm", this.rightArmPose.a());
        }

        if (!bB.equals(this.leftLegPose)) {
            nbttagcompound.set("LeftLeg", this.leftLegPose.a());
        }

        if (!bC.equals(this.rightLegPose)) {
    ...
    protected void cN() {
        List list = this.world.getEntities(this, this.getBoundingBox(), bD);
        ...
        if (itemstack1.isEmpty() || (this.bH & 1 << enumitemslot.c() + 8) == 0) {
            if (!itemstack1.isEmpty() || (this.bH & 1 << enumitemslot.c() + 16) == 0) {
                if (entityhuman.abilities.canInstantlyBuild && itemstack1.isEmpty() && !itemstack.isEmpty()) {
                    ...
                    this.setSlot(enumitemslot, itemstack3);
    ...
    private void D() {
        ...
        for(int j = 0; j < this.bF.size(); ++j) {
            ItemStack itemstack1 = this.bF.get(j);
            if (!itemstack1.isEmpty()) {
                Block.a(this.world, (new BlockPosition(this)).up(), itemstack1);
                this.bF.set(j, ItemStack.a);
            }
        }
    ...
    protected float e(float var1, float var2) {
    ...
    public void k(float fx) {
        this.aR = this.lastYaw = fx;
        this.aT = this.aS = fx;
    }

    public void setHeadRotation(float fx) {
        this.aR = this.lastYaw = fx;
        this.aT = this.aS = fx;
    }
    ...
    public void tick() {
        ...
        Vector3f vector3f1 = (Vector3f)this.datawatcher.get(c);
    ...
    public void setSmall(boolean flag) {
        this.datawatcher.set(a, this.a(this.datawatcher.get(a), 1, flag));
        this.setSize(0.5F, 1.975F);
    }
    ...
    public void setArms(boolean flag) {
        this.datawatcher.set(a, this.a(this.datawatcher.get(a), 4, flag));
    }

    public boolean hasArms() {
        return (this.datawatcher.get(a) & 4) != 0;
    }

    public void setBasePlate(boolean flag) {
        this.datawatcher.set(a, this.a(this.datawatcher.get(a), 8, flag));
    }

    public boolean hasBasePlate() {
        return (this.datawatcher.get(a) & 8) != 0;
    }

    public void setMarker(boolean flag) {
        this.datawatcher.set(a, this.a(this.datawatcher.get(a), 16, flag));
        this.setSize(0.5F, 1.975F);
    }
    ...
    protected SoundEffect m(int var1) {
