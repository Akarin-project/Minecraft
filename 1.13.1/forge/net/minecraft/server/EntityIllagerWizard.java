...
public abstract class EntityIllagerWizard extends EntityIllagerAbstract {
    ...
    protected int b;
    private EntityIllagerWizard.Spell bC = EntityIllagerWizard.Spell.NONE;
    ...
    public boolean dA() {
        if (this.world.isClientSide) {
            return this.datawatcher.get(c) > 0;
        } else {
            return this.b > 0;
        }
    }

    public void setSpell(EntityIllagerWizard.Spell entityillagerwizard$spell) {
        this.bC = entityillagerwizard$spell;
        this.datawatcher.set(c, (byte)entityillagerwizard$spell.g);
    }

    public EntityIllagerWizard.Spell getSpell() {
        return !this.world.isClientSide ? this.bC : EntityIllagerWizard.Spell.a(this.datawatcher.get(c));
    }
    ...
    public void tick() {
        ...
        if (this.world.isClientSide && this.dA()) {
            EntityIllagerWizard.Spell entityillagerwizard$spell = this.getSpell();
            double d0 = entityillagerwizard$spell.h[0];
            double d1 = entityillagerwizard$spell.h[1];
            double d2 = entityillagerwizard$spell.h[2];
            float f = this.aQ * ((float)Math.PI / 180F) + MathHelper.cos((float)this.ticksLived * 0.6662F) * 0.25F;
    ...
    public static enum Spell {
        ...
        private Spell(int j, double d0, double d1, double d2) {
            this.g = j;
            this.h = new double[]{d0, d1, d2};
        }

        public static EntityIllagerWizard.Spell a(int ix) {
            for(EntityIllagerWizard.Spell entityillagerwizard$spell : values()) {
                if (ix == entityillagerwizard$spell.g) {
                    return entityillagerwizard$spell;
                }
            }
    ...
    public abstract class c extends PathfinderGoal {
        ...
        public boolean a() {
            if (EntityIllagerWizard.this.getGoalTarget() == null) {
                return false;
            } else if (EntityIllagerWizard.this.dA()) {
                return false;
            } else {
                return EntityIllagerWizard.this.ticksLived >= this.c;
            }
        }
