...
public class WorldProviderNormal extends WorldProvider {
    ...
    public ChunkGenerator<? extends GeneratorSettings> getChunkGenerator() {
        ...
        if (worldtype == WorldType.FLAT) {
            ...
            BiomeLayoutFixedConfiguration biomelayoutfixedconfiguration1 = ((BiomeLayoutFixedConfiguration)biomelayout.b()).a(generatorsettingsflat.t());
            return chunkgeneratortype.create(this.b, biomelayout.a(biomelayoutfixedconfiguration1), generatorsettingsflat);
        } else if (worldtype == WorldType.DEBUG_ALL_BLOCK_STATES) {
            BiomeLayoutFixedConfiguration biomelayoutfixedconfiguration = ((BiomeLayoutFixedConfiguration)biomelayout.b()).a(Biomes.c);
            return chunkgeneratortype1.create(this.b, biomelayout.a(biomelayoutfixedconfiguration), chunkgeneratortype1.b());
            ...
            if (jsonobject.has("biome_source") && jsonobject.getAsJsonObject("biome_source").has("type") && jsonobject.getAsJsonObject("biome_source").has("options")) {
                ...
                if (jsonobject1.has("biomes")) {
                    ...
                    for(int i = 0; i < jsonarray.size(); ++i) {
                        BiomeBase biomebase = IRegistry.BIOME.get(new MinecraftKey(jsonarray.get(i).getAsString()));
            ...
            if (jsonobject.has("chunk_generator") && jsonobject.getAsJsonObject("chunk_generator").has("options")) {
                ...
                if (jsonobject.getAsJsonObject("chunk_generator").getAsJsonObject("options").has("default_fluid")) {
                    ...
                    Block block1 = IRegistry.BLOCK.getOrDefault(new MinecraftKey(s1));
                    if (block1 != null) {
                        iblockdata1 = block1.getBlockData();
                    }
                }
            }
    ...
    public BlockPosition a(int i, int j, boolean flag) {
        BlockPosition.MutableBlockPosition blockposition$mutableblockposition = new BlockPosition.MutableBlockPosition(i, 0, j);
        BiomeBase biomebase = this.b.getBiome(blockposition$mutableblockposition);
        ...
        if (flag && !iblockdata.getBlock().a(TagsBlock.VALID_SPAWN)) {
            ...
            if (k < 0) {
                ...
                for(int l = k + 1; l >= 0; --l) {
                    ...
                    IBlockData iblockdata1 = this.b.getType(blockposition$mutableblockposition);
                    if (!iblockdata1.s().e()) {
                        break;
                    }

                    if (iblockdata1.equals(iblockdata)) {
                        return blockposition$mutableblockposition.up().h();
                    }
                }
    ...
    public float a(long i, float f) {
        ...
        float f2 = 1.0F - (float)((Math.cos((double)f1 * Math.PI) + 1.0D) / 2.0D);
        f1 = f1 + (f2 - f1) / 3.0F;
        return f1;
    }
