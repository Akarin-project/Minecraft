...
public class EntityIllagerIllusioner extends EntityIllagerWizard implements IRangedEntity {
            ...
            if (this.hurtTicks != 1 && this.ticksLived % 1200 != 0) {
                if (this.hurtTicks == this.aC - 1) {
                    ...
                    for(int j = 0; j < 4; ++j) {
                        ...
                        this.bC[1][j] = new Vec3D(0.0D, 0.0D, 0.0D);
                    }
                }
            } else {
                this.c = 3;
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    this.bC[1][i] = new Vec3D((double)(-6.0F + (float)this.random.nextInt(13)) * 0.5D, (double)Math.max(0, this.random.nextInt(6) - 4), (double)(-6.0F + (float)this.random.nextInt(13)) * 0.5D);
                }
    ...
    public boolean r(Entity entity) {
        if (super.r(entity)) {
            return true;
        } else if (entity instanceof EntityLiving && ((EntityLiving)entity).getMonsterType() == EnumMonsterType.ILLAGER) {
            return this.be() == null && entity.be() == null;
        } else {
            return false;
        }
    }
    ...
    public void a(EntityLiving entityliving, float f) {
        ...
        entityarrow.shoot(d0, d1 + d3 * (double)0.2F, d2, 1.6F, (float)(14 - this.world.getDifficulty().a() * 4));
    ...
    class a extends EntityIllagerWizard.c {
        ...
        public boolean a() {
            if (!super.a()) {
                ...
                return false;
            } else if (EntityIllagerIllusioner.this.getGoalTarget().getId() == this.e) {
                return false;
            } else {
                return EntityIllagerIllusioner.this.world.getDamageScaler(new BlockPosition(EntityIllagerIllusioner.this)).a((float)EnumDifficulty.NORMAL.ordinal());
            }
        }
