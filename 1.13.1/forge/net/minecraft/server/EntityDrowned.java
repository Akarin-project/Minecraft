...
public class EntityDrowned extends EntityZombie implements IRangedEntity {
    ...
    public boolean a(GeneratorAccess generatoraccess, boolean flag) {
        ...
        if (biomebase != Biomes.i && biomebase != Biomes.m) {
            return this.random.nextInt(40) == 0 && this.dF() && super.a(generatoraccess, flag);
        } else {
            return this.random.nextInt(15) == 0 && super.a(generatoraccess, flag);
        }
    }
    ...
    protected boolean a(ItemStack itemstack, ItemStack itemstack1, EnumItemSlot enumitemslot) {
        if (itemstack1.getItem() == Items.NAUTILUS_SHELL) {
            return false;
        } else if (itemstack1.getItem() == Items.TRIDENT) {
            if (itemstack.getItem() == Items.TRIDENT) {
                return itemstack.getDamage() < itemstack1.getDamage();
            } else {
                return false;
            }
        } else {
            return itemstack.getItem() == Items.TRIDENT ? true : super.a(itemstack, itemstack1, enumitemslot);
        }
    }
    ...
    public boolean f(@Nullable EntityLiving entityliving) {
        if (entityliving != null) {
            return !this.world.L() || entityliving.isInWater();
        } else {
            return false;
        }
    }
    ...
    public void a(EntityLiving entityliving, float var2) {
        ...
        entitythrowntrident.shoot(d0, d1 + d3 * (double)0.2F, d2, 1.6F, (float)(14 - this.world.getDifficulty().a() * 4));
