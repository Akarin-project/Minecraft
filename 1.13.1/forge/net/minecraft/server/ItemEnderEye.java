...
public class ItemEnderEye extends Item {
    public ItemEnderEye(Item.Info item$info) {
        super(item$info);
    }
    ...
    public EnumInteractionResult a(ItemActionContext itemactioncontext) {
        ...
        if (iblockdata.getBlock() == Blocks.END_PORTAL_FRAME && !iblockdata.get(BlockEnderPortalFrame.EYE)) {
            if (world.isClientSide) {
                ...
                for(int i = 0; i < 16; ++i) {
                    double d0 = (double)((float)blockposition.getX() + (5.0F + i.nextFloat() * 6.0F) / 16.0F);
                    double d1 = (double)((float)blockposition.getY() + 0.8125F);
                    double d2 = (double)((float)blockposition.getZ() + (5.0F + i.nextFloat() * 6.0F) / 16.0F);
                    double d3 = 0.0D;
                    double d4 = 0.0D;
                    double d5 = 0.0D;
                ...
                ShapeDetector.ShapeDetectorCollection shapedetector$shapedetectorcollection = BlockEnderPortalFrame.d().a(world, blockposition);
                if (shapedetector$shapedetectorcollection != null) {
                    BlockPosition blockposition1 = shapedetector$shapedetectorcollection.a().a(-3, 0, -3);
        ...
        if (movingobjectposition != null && movingobjectposition.type == MovingObjectPosition.EnumMovingObjectType.BLOCK && world.getType(movingobjectposition.a()).getBlock() == Blocks.END_PORTAL_FRAME) {
            ...
            if (!world.isClientSide) {
                ...
                if (blockposition != null) {
                    ...
                    world.a((EntityHuman)null, entityhuman.locX, entityhuman.locY, entityhuman.locZ, SoundEffects.ENTITY_ENDER_EYE_LAUNCH, SoundCategory.NEUTRAL, 0.5F, 0.4F / (i.nextFloat() * 0.4F + 0.8F));
