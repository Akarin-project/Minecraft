...
public class WorldGenFeatureEndPlatform extends WorldGenerator<WorldGenFeatureEmptyConfiguration> {
    ...
    public boolean a(GeneratorAccess generatoraccess, ChunkGenerator<? extends GeneratorSettings> var2, Random var3, BlockPosition blockposition, WorldGenFeatureEmptyConfiguration var5) {
        ...
        if (d0 > 1024.0D) {
            ...
            BlockPosition.MutableBlockPosition blockposition$mutableblockposition = new BlockPosition.MutableBlockPosition(blockposition2);
            ...
            for(int i = blockposition.getZ(); i < blockposition.getZ() + 16; ++i) {
                for(int j = blockposition.getX(); j < blockposition.getX() + 16; ++j) {
                    if (i >= blockposition2.getZ() && i <= blockposition3.getZ() && j >= blockposition2.getX() && j <= blockposition3.getX()) {
                        blockposition$mutableblockposition.c(j, blockposition$mutableblockposition.getY(), i);
                        if (blockposition1.getX() == j && blockposition1.getZ() == i) {
                            generatoraccess.setTypeAndData(blockposition$mutableblockposition, Blocks.COBBLESTONE.getBlockData(), 2);
                        } else {
                            generatoraccess.setTypeAndData(blockposition$mutableblockposition, Blocks.STONE.getBlockData(), 2);
                        }
                    }
                }
            }
