...
public class EntityPhantom extends EntityFlying implements IMonster {
    ...
    private Vec3D b = Vec3D.a;
    private BlockPosition c = BlockPosition.ZERO;
    private EntityPhantom.AttackPhase bC = EntityPhantom.AttackPhase.CIRCLE;
    ...
    public void tick() {
        ...
        if (this.world.isClientSide) {
            float f = MathHelper.cos((float)(this.getId() * 3 + this.ticksLived) * 0.13F + (float)Math.PI);
            float f1 = MathHelper.cos((float)(this.getId() * 3 + this.ticksLived + 1) * 0.13F + (float)Math.PI);
            if (f > 0.0F && f1 <= 0.0F) {
            ...
            int i = this.getSize();
            float f2 = MathHelper.cos(this.yaw * ((float)Math.PI / 180F)) * (1.3F + 0.21F * (float)i);
            float f3 = MathHelper.sin(this.yaw * ((float)Math.PI / 180F)) * (1.3F + 0.21F * (float)i);
    ...
    public boolean b(Class<? extends EntityLiving> var1) {
        return true;
    }
    ...
    class b extends PathfinderGoal {
        ...
        public boolean a() {
            if (this.b > 0) {
                ...
                if (!list.isEmpty()) {
                    list.sort((entityhuman1, entityhuman2) -> {
                        return entityhuman1.locY > entityhuman2.locY ? -1 : 1;
                    });
    ...
    class e extends EntityPhantom.h {
        ...
        public void e() {
            ...
            if (EntityPhantom.this.random.nextInt(450) == 0) {
                this.c = EntityPhantom.this.random.nextFloat() * 2.0F * (float)Math.PI;
                this.i();
            }
        ...
        private void i() {
            ...
            this.c += this.f * 15.0F * ((float)Math.PI / 180F);
    ...
    class g extends ControllerMove {
        ...
        public void a() {
            ...
            float f6 = MathHelper.g(f4 * (180F / (float)Math.PI));
            ...
            float f7 = (float)(-(MathHelper.c((double)(-f1), d0) * (double)(180F / (float)Math.PI)));
            EntityPhantom.this.pitch = f7;
            float f8 = EntityPhantom.this.yaw + 90.0F;
            double d3 = (double)(this.j * MathHelper.cos(f8 * ((float)Math.PI / 180F))) * Math.abs((double)f / d2);
            double d4 = (double)(this.j * MathHelper.sin(f8 * ((float)Math.PI / 180F))) * Math.abs((double)f2 / d2);
            double d5 = (double)(this.j * MathHelper.sin(f7 * ((float)Math.PI / 180F))) * Math.abs((double)f1 / d2);
    ...
    class i extends EntityPhantom.h {
        ...
        public boolean b() {
            ...
            if (entityliving == null) {
                ...
                return !(entityliving instanceof EntityHuman) || !((EntityHuman)entityliving).isSpectator() && !((EntityHuman)entityliving).u() ? this.a() : false;
            }
        }
        ...
        public void e() {
            ...
            if (EntityPhantom.this.getBoundingBox().g((double)0.2F).c(entityliving.getBoundingBox())) {
