...
public class TileEntityPiston extends TileEntity implements ITickable {
    ...
    public float a(float fx) {
        ...
        return this.j + (this.i - this.j) * fx;
    }

    private float e(float fx) {
        return this.f ? fx - 1.0F : 1.0F - fx;
    }
    ...
    private void f(float fx) {
        EnumDirection enumdirection = this.h();
        double d0 = (double)(fx - this.i);
        ...
        if (!voxelshape.b()) {
            ...
            if (!list1.isEmpty()) {
                ...
                for(int ix = 0; ix < list1.size(); ++ix) {
                    Entity entity = (Entity)list1.get(ix);
                    if (entity.getPushReaction() != EnumPistonReaction.IGNORE) {
                        ...
                        for(int jx = 0; jx < list.size(); ++jx) {
                            AxisAlignedBB axisalignedbb1 = this.a(this.a((AxisAlignedBB)list.get(jx)), enumdirection, d0);
    ...
    private AxisAlignedBB a(List<AxisAlignedBB> list) {
        ...
        for(AxisAlignedBB axisalignedbb : list) {
            ...
            d5 = Math.max(axisalignedbb.f, d5);
        }
        ...
        if (this.j >= 1.0F) {
            ...
            if (this.a != null && this.world.getType(this.position).getBlock() == Blocks.MOVING_PISTON) {
                ...
                if (iblockdata.isAir()) {
                    ...
                    if (iblockdata.b(BlockProperties.y) && iblockdata.get(BlockProperties.y)) {
    ...
    public VoxelShape a(IBlockAccess iblockaccess, BlockPosition blockposition) {
        ...
        EnumDirection enumdirection = (EnumDirection)h.get();
        if ((double)this.i < 1.0D && enumdirection == this.h()) {
            ...
            float fx = this.e(this.i);
            double d0 = (double)((float)this.e.getAdjacentX() * fx);
            double d1 = (double)((float)this.e.getAdjacentY() * fx);
            double d2 = (double)((float)this.e.getAdjacentZ() * fx);
