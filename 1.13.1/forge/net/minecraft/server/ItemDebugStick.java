...
public class ItemDebugStick extends Item {
    public ItemDebugStick(Item.Info item$info) {
        super(item$info);
    }
        ...
        if (entityhuman.isCreativeAndOp()) {
            ...
            if (collection.isEmpty()) {
                ...
                if (flag) {
                    ...
                    a(entityhuman, new ChatMessage(this.getName() + ".update", new Object[]{iblockstate.a(), a(iblockdata1, iblockstate)}));
                } else {
                    iblockstate = (IBlockState)a(collection, iblockstate, entityhuman.isSneaking());
    ...
    private static <T extends Comparable<T>> IBlockData a(IBlockData iblockdata, IBlockState<T> iblockstate, boolean flag) {
        return (IBlockData)iblockdata.set(iblockstate, (Comparable)a(iblockstate.d(), iblockdata.get(iblockstate), flag));
    }

    private static <T> T a(Iterable<T> iterable, @Nullable T object, boolean flag) {
        return (T)(flag ? SystemUtils.b(iterable, object) : SystemUtils.a(iterable, object));
    }
