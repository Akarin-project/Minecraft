...
public class PathfinderGoalNearestAttackableTarget<T extends EntityLiving> extends PathfinderGoalTarget {
    ...
    public PathfinderGoalNearestAttackableTarget(EntityCreature entitycreature, Class<T> oclass, int ix, boolean flag, boolean flag1, @Nullable Predicate<? super T> predicate) {
        ...
        this.c = (entityliving) -> {
            if (entityliving == null) {
                return false;
            } else if (predicate != null && !predicate.test(entityliving)) {
                return false;
            } else {
                return !IEntitySelector.f.test(entityliving) ? false : this.a(entityliving, false);
            }
        };
    }
