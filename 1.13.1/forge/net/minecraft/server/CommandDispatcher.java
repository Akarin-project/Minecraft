...
public class CommandDispatcher {
    ...
    public CommandDispatcher(boolean flag) {
        ...
        this.b.findAmbiguities((var1, commandnode, commandnode1, collection) -> {
            a.warn("Ambiguity between arguments {} and {} with inputs: {}", this.b.getPath(commandnode), this.b.getPath(commandnode1), collection);
        });
    ...
    public void a(File file1) {
        try {
            Files.write((new GsonBuilder()).setPrettyPrinting().create().toJson(ArgumentRegistry.a(this.b, this.b.getRoot())), file1, StandardCharsets.UTF_8);
        } catch (IOException ioexception) {
            a.error("Couldn't write out command tree!", ioexception);
        }
        ...
        try {
            try {
                ...
                if (commandsyntaxexception.getInput() != null && commandsyntaxexception.getCursor() >= 0) {
                    ...
                    ichatbasecomponent.a(commandsyntaxexception.getInput().substring(Math.max(0, k - 10), k));
                    if (k < commandsyntaxexception.getInput().length()) {
                        IChatBaseComponent ichatbasecomponent1 = (new ChatComponentText(commandsyntaxexception.getInput().substring(k))).a(new EnumChatFormat[]{EnumChatFormat.RED, EnumChatFormat.UNDERLINE});
                ...
                chatcomponenttext1.<init>(exception.getMessage() == null ? exception.getClass().getName() : exception.getMessage());
                ChatComponentText chatcomponenttext = chatcomponenttext1;
                if (a.isDebugEnabled()) {
                    StackTraceElement[] astacktraceelement = exception.getStackTrace();

                    for(int j = 0; j < Math.min(astacktraceelement.length, 3); ++j) {
                        chatcomponenttext.a("\n\n").a(astacktraceelement[j].getMethodName()).a("\n ").a(astacktraceelement[j].getFileName()).a(":").a(String.valueOf(astacktraceelement[j].getLineNumber()));
                    }
                }
    ...
    public void a(EntityPlayer entityplayer) {
        ...
        this.a(this.b.getRoot(), rootcommandnode, entityplayer.getCommandListener(), hashmap);
    ...
    private void a(CommandNode<CommandListenerWrapper> commandnode, CommandNode<ICompletionProvider> commandnode1, CommandListenerWrapper commandlistenerwrapper, Map<CommandNode<CommandListenerWrapper>, CommandNode<ICompletionProvider>> map) {
        for(CommandNode commandnode2 : commandnode.getChildren()) {
            if (commandnode2.canUse(commandlistenerwrapper)) {
                ...
                argumentbuilder.requires((var0) -> {
                    return true;
                });
                if (argumentbuilder.getCommand() != null) {
                    argumentbuilder.executes((var0) -> {
                        return 0;
                    });
                }
    ...
    public static Predicate<String> a(CommandDispatcher.a commanddispatcher$a) {
        return (s) -> {
            try {
                commanddispatcher$a.parse(new StringReader(s));
                return true;
