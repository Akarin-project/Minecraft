...
public abstract class StructurePiece {
    ...
    protected abstract void a(NBTTagCompound var1, DefinedStructureManager var2);

    public void a(StructurePiece var1, List<StructurePiece> var2, Random var3) {
    }

    public abstract boolean a(GeneratorAccess var1, Random var2, StructureBoundingBox var3, ChunkCoordIntPair var4);
    ...
    protected boolean a(IBlockAccess iblockaccess, StructureBoundingBox structureboundingbox) {
        ...
        for(int k1 = i; k1 <= l; ++k1) {
            for(int l1 = k; l1 <= j1; ++l1) {
                if (iblockaccess.getType(blockposition$mutableblockposition.c(k1, j, l1)).getMaterial().isLiquid()) {
                    return true;
                }

                if (iblockaccess.getType(blockposition$mutableblockposition.c(k1, i1, l1)).getMaterial().isLiquid()) {
                    return true;
                }
            }
        }
        ...
        for(int i2 = i; i2 <= l; ++i2) {
            for(int k2 = j; k2 <= i1; ++k2) {
                if (iblockaccess.getType(blockposition$mutableblockposition.c(i2, k2, k)).getMaterial().isLiquid()) {
                    return true;
                }

                if (iblockaccess.getType(blockposition$mutableblockposition.c(i2, k2, j1)).getMaterial().isLiquid()) {
                    return true;
                }
            }
        }
        ...
        for(int j2 = k; j2 <= j1; ++j2) {
            for(int l2 = j; l2 <= i1; ++l2) {
                if (iblockaccess.getType(blockposition$mutableblockposition.c(i, l2, j2)).getMaterial().isLiquid()) {
                    return true;
                }

                if (iblockaccess.getType(blockposition$mutableblockposition.c(l, l2, j2)).getMaterial().isLiquid()) {
                    return true;
                }
            }
        }
    ...
    protected void a(GeneratorAccess generatoraccess, IBlockData iblockdata, int i, int j, int k, StructureBoundingBox structureboundingbox) {
        ...
        if (structureboundingbox.b(blockposition)) {
            ...
            if (d.contains(iblockdata.getBlock())) {
    ...
    protected IBlockData a(IBlockAccess iblockaccess, int i, int j, int k, StructureBoundingBox structureboundingbox) {
        ...
        return !structureboundingbox.b(blockposition) ? Blocks.AIR.getBlockData() : iblockaccess.getType(blockposition);
    }
    ...
    protected boolean a(IWorldReader iworldreader, int i, int j, int k, StructureBoundingBox structureboundingbox) {
        ...
        if (!structureboundingbox.b(blockposition)) {
            ...
            return i1 < iworldreader.a(HeightMap.Type.OCEAN_FLOOR_WG, l, j1);
        }
    }
    ...
    protected void a(GeneratorAccess generatoraccess, StructureBoundingBox structureboundingbox, int i, int j, int k, int l, int i1, int j1, boolean flag, Random random, StructurePiece.StructurePieceBlockSelector structurepiece$structurepieceblockselector) {
        for(int k1 = j; k1 <= i1; ++k1) {
            for(int l1 = i; l1 <= l; ++l1) {
                for(int i2 = k; i2 <= j1; ++i2) {
                    if (!flag || !this.a((IBlockAccess)generatoraccess, l1, k1, i2, structureboundingbox).isAir()) {
                        structurepiece$structurepieceblockselector.a(random, l1, k1, i2, k1 == j || k1 == i1 || l1 == i || l1 == l || i2 == k || i2 == j1);
                        this.a(generatoraccess, structurepiece$structurepieceblockselector.a(), l1, k1, i2, structureboundingbox);
                    }
                }
            }
        }
    ...
    protected boolean a(GeneratorAccess generatoraccess, StructureBoundingBox structureboundingbox, Random random, BlockPosition blockposition, MinecraftKey minecraftkey, @Nullable IBlockData iblockdata) {
        if (structureboundingbox.b(blockposition) && generatoraccess.getType(blockposition).getBlock() != Blocks.CHEST) {
            if (iblockdata == null) {
                iblockdata = a(generatoraccess, blockposition, Blocks.CHEST.getBlockData());
            }
    ...
    protected boolean a(GeneratorAccess generatoraccess, StructureBoundingBox structureboundingbox, Random random, int i, int j, int k, EnumDirection enumdirection, MinecraftKey minecraftkey) {
        BlockPosition blockposition = new BlockPosition(this.a(i, k), this.d(j), this.b(i, k));
        if (structureboundingbox.b(blockposition) && generatoraccess.getType(blockposition).getBlock() != Blocks.DISPENSER) {
    ...
    protected void a(GeneratorAccess generatoraccess, StructureBoundingBox structureboundingbox, Random var3, int i, int j, int k, EnumDirection enumdirection, BlockDoor blockdoor) {
