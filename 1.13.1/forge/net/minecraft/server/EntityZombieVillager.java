...
public class EntityZombieVillager extends EntityZombie {
    ...
    public int getProfession() {
        return Math.max(this.datawatcher.get(b) % 6, 0);
    }
    ...
    protected void a(@Nullable UUID uuid, int i) {
        this.bD = uuid;
        this.conversionTime = i;
        this.getDataWatcher().set(a, true);
            ...
            for(int k = (int)this.locX - 4; k < (int)this.locX + 4 && j < 14; ++k) {
                for(int l = (int)this.locY - 4; l < (int)this.locY + 4 && j < 14; ++l) {
                    for(int i1 = (int)this.locZ - 4; i1 < (int)this.locZ + 4 && j < 14; ++i1) {
                        Block block = this.world.getType(blockposition$mutableblockposition.c(k, l, i1)).getBlock();
    ...
    public SoundEffect d(DamageSource var1) {
