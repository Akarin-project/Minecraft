...
public class WorldGenMonument extends StructureGenerator<WorldGenMonumentConfiguration> {
    ...
    protected ChunkCoordIntPair a(ChunkGenerator<?> chunkgenerator, Random random, int i, int j, int k, int l) {
        ...
        k2 = k2 * i1;
        l2 = l2 * i1;
        k2 = k2 + (random.nextInt(i1 - j1) + random.nextInt(i1 - j1)) / 2;
        l2 = l2 + (random.nextInt(i1 - j1) + random.nextInt(i1 - j1)) / 2;
    ...
    protected boolean a(ChunkGenerator<?> chunkgenerator, Random random, int i, int j) {
        ...
        if (i == chunkcoordintpair.x && j == chunkcoordintpair.z) {
            for(BiomeBase biomebase : chunkgenerator.getWorldChunkManager().a(i * 16 + 9, j * 16 + 9, 16)) {
                if (!chunkgenerator.canSpawnStructure(biomebase, WorldGenerator.n)) {
                    return false;
                }
            }

            for(BiomeBase biomebase1 : chunkgenerator.getWorldChunkManager().a(i * 16 + 9, j * 16 + 9, 29)) {
                if (biomebase1.p() != BiomeBase.Geography.OCEAN && biomebase1.p() != BiomeBase.Geography.RIVER) {
                    return false;
                }
            }
