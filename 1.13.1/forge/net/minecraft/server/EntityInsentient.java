...
public abstract class EntityInsentient extends EntityLiving {
    ...
    private final NonNullList<ItemStack> bD = NonNullList.<ItemStack>a(2, ItemStack.a);
    public float[] dropChanceHand = new float[2];
    private final NonNullList<ItemStack> bE = NonNullList.<ItemStack>a(4, ItemStack.a);
    public float[] dropChanceArmor = new float[4];
    public boolean canPickUpLoot;
    public boolean persistent;
    private final Map<PathType, Float> bH = Maps.newEnumMap(PathType.class);
    ...
    public float a(PathType pathtype) {
        Float f = (Float)this.bH.get(pathtype);
        return f == null ? pathtype.a() : f;
    }
    ...
    protected int getExpValue(EntityHuman var1) {
        if (this.b_ > 0) {
            ...
            for(int j = 0; j < this.bE.size(); ++j) {
                if (!((ItemStack)this.bE.get(j)).isEmpty() && this.dropChanceArmor[j] <= 1.0F) {
                    i += 1 + this.random.nextInt(3);
                }
            }

            for(int k = 0; k < this.bD.size(); ++k) {
                if (!((ItemStack)this.bD.get(k)).isEmpty() && this.dropChanceHand[k] <= 1.0F) {
                    i += 1 + this.random.nextInt(3);
                }
            }
    ...
    protected float e(float var1, float f) {
        this.b.a();
        return f;
    }
    ...
    protected void dropDeathLoot(boolean var1, int i) {
    ...
    public void b(NBTTagCompound nbttagcompound) {
        ...
        for(ItemStack itemstack : this.bE) {
        ...
        for(ItemStack itemstack1 : this.bD) {
        ...
        for(float f1 : this.dropChanceHand) {
    ...
    public void a(NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.hasKeyOfType("HandItems", 9)) {
            NBTTagList nbttaglist1 = nbttagcompound.getList("HandItems", 10);

            for(int j = 0; j < this.bD.size(); ++j) {
                this.bD.set(j, ItemStack.a(nbttaglist1.getCompound(j)));
            }
        }

        if (nbttagcompound.hasKeyOfType("ArmorDropChances", 9)) {
            NBTTagList nbttaglist2 = nbttagcompound.getList("ArmorDropChances", 5);

            for(int k = 0; k < nbttaglist2.size(); ++k) {
                this.dropChanceArmor[k] = nbttaglist2.l(k);
            }
        }

        if (nbttagcompound.hasKeyOfType("HandDropChances", 9)) {
            NBTTagList nbttaglist3 = nbttagcompound.getList("HandDropChances", 5);

            for(int l = 0; l < nbttaglist3.size(); ++l) {
                this.dropChanceHand[l] = nbttaglist3.l(l);
            }
        }
    ...
    protected void a(boolean flag, int i, DamageSource damagesource) {
        ...
        if (minecraftkey != null) {
            ...
            LootTableInfo.Builder loottableinfo$builder = (new LootTableInfo.Builder((WorldServer)this.world)).entity(this).damageSource(damagesource).position(new BlockPosition(this));
            if (flag && this.killer != null) {
                loottableinfo$builder = loottableinfo$builder.killer(this.killer).luck(this.killer.dJ());
            }

            for(ItemStack itemstack : loottable.a(this.lootTableSeed == 0L ? this.random : new Random(this.lootTableSeed), loottableinfo$builder.build())) {
                this.a_(itemstack);
            }
    ...
    protected boolean d(ItemStack var1) {
        return true;
    }
    ...
    protected final void doTick() {
        ...
        if (this.isPassenger() && this.getVehicle() instanceof EntityInsentient) {
            EntityInsentient entityinsentient1 = (EntityInsentient)this.getVehicle();
            entityinsentient1.getNavigation().a(this.getNavigation().m(), 1.5D);
            entityinsentient1.getControllerMove().a(this.getControllerMove());
        }
    ...
    public void a(Entity entity, float f, float f1) {
        ...
        if (entity instanceof EntityLiving) {
            ...
            d1 = entityliving.locY + (double)entityliving.getHeadHeight() - (this.locY + (double)this.getHeadHeight());
        } else {
            d1 = (entity.getBoundingBox().b + entity.getBoundingBox().e) / 2.0D - (this.locY + (double)this.getHeadHeight());
        }

        double d3 = (double)MathHelper.sqrt(d0 * d0 + d2 * d2);
        float f2 = (float)(MathHelper.c(d2, d0) * (double)(180F / (float)Math.PI)) - 90.0F;
        float f3 = (float)(-(MathHelper.c(d1, d3) * (double)(180F / (float)Math.PI)));
    ...
    public ItemStack getEquipment(EnumItemSlot enumitemslot) {
        switch(enumitemslot.a()) {
        case HAND:
            return this.bD.get(enumitemslot.b());
        case ARMOR:
            return this.bE.get(enumitemslot.b());
        default:
            return ItemStack.a;
        }
    }
    ...
    public static EnumItemSlot e(ItemStack itemstack) {
        ...
        if (item != Blocks.CARVED_PUMPKIN.getItem() && (!(item instanceof ItemBlock) || !(((ItemBlock)item).getBlock() instanceof BlockSkullAbstract))) {
            if (item instanceof ItemArmor) {
                return ((ItemArmor)item).b();
            } else if (item == Items.ELYTRA) {
                return EnumItemSlot.CHEST;
            } else {
                return item == Items.SHIELD ? EnumItemSlot.OFFHAND : EnumItemSlot.MAINHAND;
            }
        } else {
    ...
    @Nullable
    public GroupDataEntity prepare(DifficultyDamageScaler var1, @Nullable GroupDataEntity groupdataentity, @Nullable NBTTagCompound var3) {
    ...
    protected boolean a(EntityHuman var1, EnumHand var2) {
        return false;
    }
    ...
    private void dr() {
        if (this.bK && this.bM != null) {
            if (this.bM.b("UUID")) {
                ...
                for(EntityLiving entityliving : this.world.a(EntityLiving.class, this.getBoundingBox().g(10.0D))) {
    ...
    public void setNoAI(boolean flag) {
        ...
        this.datawatcher.set(a, flag ? (byte)(b0 | 1) : (byte)(b0 & -2));
    }

    public void r(boolean flag) {
        byte b0 = this.datawatcher.get(a);
        this.datawatcher.set(a, flag ? (byte)(b0 | 2) : (byte)(b0 & -3));
    }
    ...
    public boolean isLeftHanded() {
        return (this.datawatcher.get(a) & 2) != 0;
    }
