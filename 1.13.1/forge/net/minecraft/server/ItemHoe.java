...
public class ItemHoe extends ItemToolMaterial {
    ...
    public ItemHoe(ToolMaterial toolmaterial, float f, Item.Info item$info) {
    ...
    public EnumInteractionResult a(ItemActionContext itemactioncontext) {
        ...
        if (itemactioncontext.getClickedFace() != EnumDirection.DOWN && world.getType(blockposition.up()).isAir()) {
            IBlockData iblockdata = (IBlockData)a.get(world.getType(blockposition).getBlock());
            if (iblockdata != null) {
    ...
    public Multimap<String, AttributeModifier> a(EnumItemSlot enumitemslot) {
        ...
        if (enumitemslot == EnumItemSlot.MAINHAND) {
            multimap.put(GenericAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier(g, "Weapon modifier", 0.0D, 0));
