...
public class CommandEnchant {
    ...
    public static void a(com.mojang.brigadier.CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        commanddispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandDispatcher.a("enchant").requires((commandlistenerwrapper) -> {
            return commandlistenerwrapper.hasPermission(2);
        })).then(CommandDispatcher.a("targets", ArgumentEntity.b()).then(((RequiredArgumentBuilder)CommandDispatcher.a("enchantment", ArgumentEnchantment.a()).executes((commandcontext) -> {
        ...
        })).then(CommandDispatcher.a("level", IntegerArgumentType.integer(0)).executes((commandcontext) -> {
    ...
    private static int a(CommandListenerWrapper commandlistenerwrapper, Collection<? extends Entity> collection, Enchantment enchantment, int i) throws CommandSyntaxException {
        if (i > enchantment.getMaxLevel()) {
            throw d.create(i, enchantment.getMaxLevel());
        } else {
            int j = 0;
            ...
            for(Entity entity : collection) {
                if (entity instanceof EntityLiving) {
                    ...
                    if (!itemstack.isEmpty()) {
                        if (enchantment.canEnchant(itemstack) && EnchantmentManager.a(EnchantmentManager.a(itemstack).keySet(), enchantment)) {
                            itemstack.addEnchantment(enchantment, i);
                            ++j;
                        } else if (collection.size() == 1) {
                            throw c.create(itemstack.getItem().i(itemstack).getString());
                        }
                    } else if (collection.size() == 1) {
                        throw b.create(entityliving.getDisplayName().getString());
                    }
                } else if (collection.size() == 1) {
                    throw a.create(entity.getDisplayName().getString());
                }
            }
