...
public class ChatDeserializer {
    ...
    public static Item b(JsonElement jsonelement, String s) {
        if (jsonelement.isJsonPrimitive()) {
            ...
            Item item = IRegistry.ITEM.get(new MinecraftKey(s1));
            if (item == null) {
    ...
    public static <T> T a(JsonObject jsonobject, String s, JsonDeserializationContext jsondeserializationcontext, Class<? extends T> oclass) {
        if (jsonobject.has(s)) {
            return (T)a(jsonobject.get(s), s, jsondeserializationcontext, oclass);
        } else {
            throw new JsonSyntaxException("Missing " + s);
        }
    }

    public static <T> T a(JsonObject jsonobject, String s, T object, JsonDeserializationContext jsondeserializationcontext, Class<? extends T> oclass) {
        return (T)(jsonobject.has(s) ? a(jsonobject.get(s), s, jsondeserializationcontext, oclass) : object);
    }
