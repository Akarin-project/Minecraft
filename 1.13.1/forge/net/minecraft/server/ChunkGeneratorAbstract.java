...
public abstract class ChunkGeneratorAbstract<C extends GeneratorSettings> implements ChunkGenerator<C> {
    ...
    public void addFeatures(RegionLimitedWorldAccess regionlimitedworldaccess, WorldGenStage.Features worldgenstage$features) {
        SeededRandom seededrandom = new SeededRandom(this.b);
        boolean flag = true;
        int i = regionlimitedworldaccess.a();
        int j = regionlimitedworldaccess.b();
        BitSet bitset = regionlimitedworldaccess.b(i, j).a(worldgenstage$features);

        for(int k = i - 8; k <= i + 8; ++k) {
            for(int l = j - 8; l <= j + 8; ++l) {
                List list = regionlimitedworldaccess.getChunkProvider().getChunkGenerator().getWorldChunkManager().getBiome(new BlockPosition(k * 16, 0, l * 16), (BiomeBase)null).a(worldgenstage$features);
    ...
    protected void a(IChunkAccess ichunkaccess, Random random) {
        ...
        for(BlockPosition blockposition : BlockPosition.a(i, 0, j, i + 16, 0, j + 16)) {
            for(int k = 4; k >= 0; --k) {
                if (k <= random.nextInt(5)) {
                    ichunkaccess.a(blockposition$mutableblockposition.c(blockposition.getX(), k, blockposition.getZ()), Blocks.BEDROCK.getBlockData(), false);
                }
            }
        }
    ...
    public void addDecorations(RegionLimitedWorldAccess regionlimitedworldaccess) {
        ...
        for(WorldGenStage.Decoration worldgenstage$decoration : WorldGenStage.Decoration.values()) {
            biomebase.a(worldgenstage$decoration, this, regionlimitedworldaccess, i1, seededrandom, blockposition);
        }
    ...
    public Long2ObjectMap<StructureStart> getStructureStartCache(StructureGenerator<? extends WorldGenFeatureConfiguration> structuregenerator) {
        return (Long2ObjectMap)this.d.computeIfAbsent(structuregenerator, (var0) -> {
    ...
    public Long2ObjectMap<LongSet> getStructureCache(StructureGenerator<? extends WorldGenFeatureConfiguration> structuregenerator) {
        return (Long2ObjectMap)this.e.computeIfAbsent(structuregenerator, (var0) -> {
            return Long2ObjectMaps.synchronize(new ExpiringMap(8192, 10000));
        });
    }
