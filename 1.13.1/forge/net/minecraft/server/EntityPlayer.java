...
public class EntityPlayer extends EntityHuman implements ICrafting {
    ...
    public void b(NBTTagCompound nbttagcompound) {
        ...
        if (entity != null && entity1 != this && entity1.bR()) {
            NBTTagCompound nbttagcompound2 = new NBTTagCompound();
            NBTTagCompound nbttagcompound3 = new NBTTagCompound();
            entity1.d(nbttagcompound3);
            nbttagcompound2.a("Attach", entity.getUniqueID());
    ...
    public void a(int i) {
        float fx = (float)this.getExpToLevel();
        float f1 = (fx - 1.0F) / fx;
        this.exp = MathHelper.a((float)i / fx, 0.0F, f1);
        this.lastSentExp = -1;
    }
        ...
        if (flag) {
            ...
            this.playerConnection.a(new PacketPlayOutCombatEvent(this.getCombatTracker(), PacketPlayOutCombatEvent.EnumCombatEventType.ENTITY_DIED, ichatbasecomponent), (future) -> {
                if (!future.isSuccess()) {
                    ...
                    IChatBaseComponent ichatbasecomponent2 = (new ChatMessage("death.attack.even_more_magic", new Object[]{this.getScoreboardDisplayName()})).a((chatmodifier) -> {
                        chatmodifier.setChatHoverable(new ChatHoverable(ChatHoverable.EnumHoverAction.SHOW_TEXT, chatmessage));
                    });
                    this.playerConnection.sendPacket(new PacketPlayOutCombatEvent(this.getCombatTracker(), PacketPlayOutCombatEvent.EnumCombatEventType.ENTITY_DIED, ichatbasecomponent2));
                }
    ...
    public boolean damageEntity(DamageSource damagesource, float fx) {
        if (this.isInvulnerable(damagesource)) {
            ...
            if (!flag && this.invulnerableTicks > 0 && damagesource != DamageSource.OUT_OF_WORLD) {
                ...
                return super.damageEntity(damagesource, fx);
            }
        }
    }
    ...
    public boolean a(EntityPlayer entityplayer1) {
        if (entityplayer1.isSpectator()) {
            ...
            return this.isSpectator() ? false : super.a(entityplayer1);
        }
    }
    ...
    public EntityHuman.EnumBedResult a(BlockPosition blockposition) {
        EntityHuman.EnumBedResult entityhuman$enumbedresult = super.a(blockposition);
        if (entityhuman$enumbedresult == EntityHuman.EnumBedResult.OK) {
            ...
            this.getWorldServer().getTracker().a(this, packetplayoutbed);
    ...
    public void a(float fx, float f1, boolean flag, boolean flag1) {
        if (this.isPassenger()) {
            if (fx >= -1.0F && fx <= 1.0F) {
                this.bh = fx;
            }
    ...
    public void a(ArgumentAnchor.Anchor argumentanchor$anchor, Vec3D vec3d) {
        ...
        this.playerConnection.sendPacket(new PacketPlayOutLookAt(argumentanchor$anchor, vec3d.x, vec3d.y, vec3d.z));
    }

    public void a(ArgumentAnchor.Anchor argumentanchor$anchor, Entity entity, ArgumentAnchor.Anchor argumentanchor$anchor1) {
        Vec3D vec3d = argumentanchor$anchor1.a(entity);
        super.a(argumentanchor$anchor, vec3d);
        this.playerConnection.sendPacket(new PacketPlayOutLookAt(argumentanchor$anchor, entity, argumentanchor$anchor1));
    }

    public void copyFrom(EntityPlayer entityplayer1, boolean flag) {
        if (flag) {
            this.inventory.a(entityplayer1.inventory);
            this.setHealth(entityplayer1.getHealth());
            this.foodData = entityplayer1.foodData;
            this.expLevel = entityplayer1.expLevel;
            this.expTotal = entityplayer1.expTotal;
            this.exp = entityplayer1.exp;
            this.setScore(entityplayer1.getScore());
            this.aq = entityplayer1.aq;
            this.ar = entityplayer1.ar;
            this.as = entityplayer1.as;
        } else if (this.world.getGameRules().getBoolean("keepInventory") || entityplayer1.isSpectator()) {
            this.inventory.a(entityplayer1.inventory);
            this.expLevel = entityplayer1.expLevel;
            this.expTotal = entityplayer1.expTotal;
            this.exp = entityplayer1.exp;
        ...
        this.bZ = entityplayer1.bZ;
        this.enderChest = entityplayer1.enderChest;
        this.getDataWatcher().set(bx, entityplayer1.getDataWatcher().get(bx));
        this.lastSentExp = -1;
        this.lastHealthSent = -1.0F;
        this.co = -1;
        this.cy.a(entityplayer1.cy);
        this.removeQueue.addAll(entityplayer1.removeQueue);
        this.cx = entityplayer1.cx;
        this.cC = entityplayer1.cC;
        this.setShoulderEntityLeft(entityplayer1.getShoulderEntityLeft());
        this.setShoulderEntityRight(entityplayer1.getShoulderEntityRight());
    }
    ...
    public void a(IChatBaseComponent ichatbasecomponent, ChatMessageType chatmessagetype) {
        this.playerConnection.a(new PacketPlayOutChat(ichatbasecomponent, chatmessagetype), (future) -> {
            if (!future.isSuccess() && (chatmessagetype == ChatMessageType.GAME_INFO || chatmessagetype == ChatMessageType.SYSTEM)) {
                boolean flag = true;
                String s = ichatbasecomponent.a(256);
                IChatBaseComponent ichatbasecomponent2 = (new ChatComponentText(s)).a(EnumChatFormat.YELLOW);
                this.playerConnection.sendPacket(new PacketPlayOutChat((new ChatMessage("multiplayer.message_not_delivered", new Object[]{ichatbasecomponent2})).a(EnumChatFormat.RED), ChatMessageType.SYSTEM));
            }
    ...
    public void a(PacketPlayInSettings packetplayinsettings) {
        ...
        this.getDataWatcher().set(bx, (byte)packetplayinsettings.f());
        this.getDataWatcher().set(by, (byte)(packetplayinsettings.getMainHand() == EnumMainHand.LEFT ? 0 : 1));
    }
    ...
    public void a(WorldServer worldserver, double d0, double d1, double d2, float fx, float f1) {
        this.setSpectatorTarget(this);
        this.stopRiding();
        if (worldserver == this.world) {
            this.playerConnection.a(d0, d1, d2, fx, f1);
        } else {
            ...
            this.dead = false;
            this.setPositionRotation(d0, d1, d2, fx, f1);
            if (this.isAlive()) {
